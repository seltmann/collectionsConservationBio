...
##### Example For Loop to pull out individual families and calculate network metrics 
```{r}
## dataframe to matrix
web <- function(df) {
  as.data.frame(frame2webs(df, varnames = c("host", "insect", "h_family", "coll_number_same_h"), type.out = 'array'))
...
# Dennstaedtiaceae <- 16 obs
unique_families <- unique(data$h_family) 
unique_families
...
family_data <- function(i) {
  den <- subset(data_new, data_new$h_family == i)
  return(den)
}
...
for (i in unique_families) {
  print(i)
  den <- family_data(i)
  x <- multi_metric(web(den))
  den_output <- (rbind(den_output,x))
}
