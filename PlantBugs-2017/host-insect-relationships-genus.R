...
dbListFields(connection, "Locality")
...
headers <- paste("i_genus","i_species","dlat","dlong","georef_method","GeoRefAccuracy","ElevM","country","stateProv","collectingID","h_family","h_genus","h_species","i_family","i_tribe","i_genus","i_species","h_family_id","h_genus_id","h_species_id","i_family_id","i_tribe_id","i_genus_id","i_species_id", sep = "\t" )
...
rs <- dbSendQuery(connection,"Select T1.TaxName as genus,T2.TaxName as species, L1.DLat as dlat, L1.DLong as dlong, L1.GeoRefMethod as geoMethod, L1.LocAccuracy as location_accuracy,L1.ElevM as elevationM,CN.Country as country,SP.StateProv as stateProv,S1.ColEventUID as eventID,F4.HostTaxName as h_family,
                  F1.HostTaxName as h_genus,F2.HostTaxName as h_species, T5.TaxName as i_family,T3.TaxName as i_tribe,T1.TaxName as i_genus,
                  T2.TaxName as i_species,F4.HostMNLUID as h_family_id,F1.HostMNLUID as h_genus_id,F2.HostMNLUID as h_species_id, T5.MNLUID as i_family_id,
                  T3.MNLUID as i_tribe_id,T1.MNLUID as i_genus_id,T2.MNLUID as i_species_id
                  FROM Specimen S1 
                  ...
                  left join Collector C1 on CE.Collector=C1.CollectorUID 
                  left join Country CN on SP.CountryUID=CN.UID 
                  WHERE T5.TaxName = 'Miridae' AND T1.TaxName not like '%sp.%' AND T2.TaxName not like '%sp.%' AND (CN.UID = '2' or CN.UID = '8' or CN.UID = '11')")
